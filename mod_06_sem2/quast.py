...
def main(args):
    ...
    except (ImportError, AttributeError):        reload(qconfig)
        reload(qutils)
    ...
    if ref_fpath:
        ...
        original_ref_fpath = ref_fpath        ref_fpath = qutils.correct_reference(ref_fpath, corrected_dirpath)
    ...
    if len(aligned_contigs_fpaths) and ref_fpath:
        ...
        ########################################################################
        from quast_libs import aligned_stats        aligned_stats.do(
    ...
    if qconfig.html_report:
        ...
        html_saver.save_colors(output_dirpath, contigs_fpaths, plotter_data.dict_color_and_ls)        html_saver.save_total_report(output_dirpath, qconfig.min_contig, ref_fpath)
